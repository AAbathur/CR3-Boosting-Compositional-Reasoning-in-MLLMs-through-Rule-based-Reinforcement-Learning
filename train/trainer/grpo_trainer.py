...
class Qwen2VLGRPOTrainerForCompositionality(Qwen2VLGRPOTrainer):
    def __init__(
        ...
        self.type2count = {} ## 每种type对应的count,total
        super().__init__(
            model=model,
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                reward_kwargs = {key: [] for key in inputs[0].keys() if key not in ["prompt", "completion"]} ## 应该是 image, problem, solution三个key
                ...
                rewards_per_func[:, i] = torch.tensor(output_reward_func, dtype=torch.float32, device=device) ## rewards_per_func shape是(bs, num_reward_func); 每列是对应reward给每个样本计算出来的reward

        # Gather rewards across processes
        rewards_per_func = self.accelerator.gather(rewards_per_func)
        
        # Sum the rewards from all reward functions
        rewards = rewards_per_func.sum(dim=1) ## 每个样本在不同reward函数上的结果相加
        
        # Compute grouped-wise rewards
        # Each group consists of num_generations completions for the same prompt
        mean_grouped_rewards = rewards.view(-1, self.num_generations).mean(dim=1) ## 同一个样本的num_generation个生成结果计算均值，要配合对应的数据集的RepeatSampler
...
class InternVL3Trainer(Trainer):
    ...
    # Args
        ...
        ## is deepspeed zero3 enable()是True
        ...
        for i, reward_func in enumerate(reward_funcs):
            ### isinstance(reward_func, str)是False
        ...
        for i, (reward_processing_class, reward_func) in enumerate(zip(reward_processing_classes, reward_funcs)):
            ### isinstance(reward_func, PreTrainedModel)是False, iou_reward和format_reward都是函数
            if isinstance(reward_func, PreTrainedModel):
        ...
        # Multi-step
        self.num_iterations = args.num_iterations  # = 𝜇 in the GRPO paper ## 1
        ...
        ### self.ref_model is not None 且 self.is_deepspeed_enabled是True
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        for x in inputs:
            if "image" in x:
                ...
                one_data_img = [] ## 每个数据中的图像单独记录在一个list中
                for imgx in img:
        ...
        for text, npatchx in zip(prompts_text, num_patches_list):
            for npatch in npatchx:
                ...
                text = text.replace("<image>", image_tokens, 1) ## 每次替换一个<image>
        ...
        is_eos = completion_ids == self.generation_config['eos_token_id'] ## 生成的eos token和tokenizer中的eos token不是一个
        ...
        new_prompt_completion_ids = torch.cat([prompt_ids, prompt_completion_ids], dim=1) ## qwen2.5-vl的prompt_completion_ids是包含input_ids的
        ...
        #if is_conversational(inputs[0]): ## 我们对输入做了特殊处理，所以is_conversational这里检查是False，直接注释掉
        completions = [[{"role": "assistant", "content": completion}] for completion in completions]
